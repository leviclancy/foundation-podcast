...
echo "<amp-lightbox id='lightbox-add-episode' on=\"lightboxOpen:".$lightbox_close_array.",AMP.setState({addEpisodeBack: 'Back', addEpisodeLabel: ''}),add-episode-form.clear;lightboxClose:add-episode-form.clear,".$lightbox_close_array."\" layout='nodisplay' scrollable>";
	...
	echo "<label class='form-label' id='add-episode-label' for='add-episode' [text]=\"addEpisodeLabel.slice(-1) == '' ? 'Click to add MP3 file.' : addEpisodeLabel.slice(-1)\"></label>";
	echo "<input type='file' id='add-episode' name='add-episode' placeholder='Add MP3 file' accept='.mp3,audio/mpeg3' on=\"input-throttled:AMP.setState({addEpisodeBack: 'Back without adding'});change:AMP.setState({addEpisodeLabel: event.value.split('\')})\" hidden>";
